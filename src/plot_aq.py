...
def plot_diff_facet(params, csv_pattern, start_date='2013-05-01', end_date='2013-06-01'):
    ...
    for p in params:
        ...
        df['Periodic Q-Digest'] = np.abs(df['f_approx'] - df['f_central']) / df['f_central'] * 100.0
        df['Event-Driven']      = np.abs(df['f_event']  - df['f_central']) / df['f_central'] * 100.0
        df['Untrimmed Mean']    = np.abs(df['f_mean']   - df['f_central']) / df['f_central'] * 100.0
        # Melt to long form
        df_long = df.melt(
            id_vars=['time'],
            value_vars=['Periodic Q-Digest', 'Event-Driven', 'Untrimmed Mean'],
            var_name='Method',
    ...
    g.map(sns.lineplot, 'time', 'Percentage Error')
